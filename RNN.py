# !/usr/bin/env python
# title           :RNN
# description     :RNN Model That Predicts Closing Price of Stock at T+1
# author          :Juan Maldonado
# date            :5/2/19
# version         :1.0
# usage           :python3 RNN.py
...
# PARAMETERS (Will adjust be for the purpose of producing differing prediction results)
...
# Layer Design: RNN APPROACH


layers = [tf.contrib.rnn.BasicRNNCell(num_units=num_neurons, activation=tf.nn.elu) for layer in range(num_layers)]
...
plt.title('RNN : MSE  Tst: %.6f / Vld: %.6f | Epochs : %d | LRate : %.3f | Nrns: %d | Lyrs : %d | BSize : %d' %
          (mean_squared_error(y_test, y_testing_pred), mean_squared_error(y_validation, y_validation_pred),
           num_epochs, learning_rate, num_neurons, num_layers, batch_size))
plt.ylabel("Price")
plt.xlabel("Time")
...
plt.legend()
#plt.show()
figure.savefig('RNN/RNNBestConfig' + '.png')
